...
int main(int argc, char **argv) {
  ...
  SDL_Window *window = secp(SDL_CreateWindow(
      "sowon", 0, 0, TEXT_WIDTH, TEXT_HEIGHT * 2, SDL_WINDOW_RESIZABLE));
  ...
  secc(
      SDL_SetTextureColorMod(digits, MAIN_COLOR_R, MAIN_COLOR_G, MAIN_COLOR_B));

  if (paused) {
    secc(SDL_SetTextureColorMod(digits, PAUSE_COLOR_R, PAUSE_COLOR_G,
                                PAUSE_COLOR_B));
  } else {
    secc(SDL_SetTextureColorMod(digits, MAIN_COLOR_R, MAIN_COLOR_G,
                                MAIN_COLOR_B));
  }
  ...
  while (!quit) {
    ...
    SDL_RenderClear(renderer);
    {
...
#ifdef PENGER
      render_penger_at(renderer, penger, displayed_time, mode == MODE_COUNTDOWN,
                       window);
#endif
      ...
      render_digit_at(renderer, digits, hours / 10, wiggle_index % WIGGLE_COUNT,
                      &pen_x, &pen_y, user_scale, fit_scale);
      render_digit_at(renderer, digits, hours % 10,
                      (wiggle_index + 1) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);
      render_digit_at(renderer, digits, COLON_INDEX,
                      wiggle_index % WIGGLE_COUNT, &pen_x, &pen_y, user_scale,
                      fit_scale);

      const size_t minutes = t / 60 % 60;
      render_digit_at(renderer, digits, minutes / 10,
                      (wiggle_index + 2) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);
      render_digit_at(renderer, digits, minutes % 10,
                      (wiggle_index + 3) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);
      render_digit_at(renderer, digits, COLON_INDEX,
                      (wiggle_index + 1) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);
      ...
      render_digit_at(renderer, digits, seconds / 10,
                      (wiggle_index + 4) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);
      render_digit_at(renderer, digits, seconds % 10,
                      (wiggle_index + 5) % WIGGLE_COUNT, &pen_x, &pen_y,
                      user_scale, fit_scale);

      char title[TITLE_CAP];
      snprintf(title, sizeof(title), "%02zu:%02zu:%02zu - sowon", hours,
               minutes, seconds);
    ...
    if (!paused) {
      ...
      case MODE_CLOCK: {
        ...
        displayed_time =
            tm->tm_sec + tm->tm_min * 60.0f + tm->tm_hour * 60.0f * 60.0f;
        if (displayed_time <= displayed_time_prev) {
          // same second, keep previous count and add subsecond resolution for
          // penger
          if (floorf(displayed_time_prev) ==
              floorf(
                  displayed_time_prev +
                  fps_dt.dt)) { // check for no newsecond shenaningans from dt
